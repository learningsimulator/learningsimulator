...
'''
        ...
        for _, plot in plot_data.items():
            ...
            for i in range(1, len(y_values)):
                if y_values[i] < y_values[i - 1]:
                    self.assertLess(y_values[i - 1] - y_values[i], 0.05)
        ...
        for y in ypa50:
            self.assertLess(y, 0)
        ypa50_last = ypa50[-50:-1]  # The last 50 items, exept the last
        for y in ypa50_last:
        ...
        self.assertLess(max(y0_last) - min(y0_last), 0.05)

        # Check that "0.25" converges to a value between 9 and 9.5
        y25 = plot_data['0.25']['y']
        for y in y25:
            self.assertLess(y, 9.5)
        y25_last = y25[-50:-1]
        for y in y25_last:
            self.assertGreater(y, 9)
        self.assertLess(max(y25_last) - min(y25_last), 0.01)

        # Check that "0.50" converges to a value between 3 and 4
        y50 = plot_data['0.5']['y']
        for y in y50:
            self.assertLess(y, 7)
        y50_last = y50[-50:-1]
        for y in y50_last:
            self.assertGreater(y, 6)
        self.assertLess(max(y50_last) - min(y50_last), 0.1)

        # Check that "0.75" converges to a value between 3 and 4
        ...
        self.assertLess(max(y75_last) - min(y75_last), 0.2)
